...
function setFilter(filter) {
    ...
    ['day', 'week', 'month', 'year', 'all'].forEach(f => {
        ...
        btn.className = `px-4 py-2 rounded-lg text-sm font-semibold ${f === filter ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-[#2a2a2a] text-gray-700 dark:text-gray-400'}`;
    });
    renderSales();
}
...
function filterSales() {
    ...
    return allSales.filter(sale => {
        ...
        if (currentFilter === 'day') return date.toDateString() === now.toDateString();
        else if (currentFilter === 'week') {
            const weekAgo = new Date(now);
            weekAgo.setDate(now.getDate() - 7);
            return date >= weekAgo;
        } else if (currentFilter === 'month') return date.getMonth() === now.getMonth() && date.getFullYear() === now.getFullYear();
        else if (currentFilter === 'year') return date.getFullYear() === now.getFullYear();
        return true;
    });
}
...
function getPrintHeader() {
    ...
    const labels = {
        day: { title: 'Reporte diario', subtitle: now.toLocaleDateString('es-MX', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' }) },
        week: { title: 'Reporte semanal', subtitle: `${from} — ${to}` },
        month: { title: 'Reporte mensual', subtitle: now.toLocaleDateString('es-MX', { month: 'long', year: 'numeric' }) },
        year: { title: `Reporte anual ${now.getFullYear()}`, subtitle: `Enero — ${now.toLocaleDateString('es-MX', { month: 'long' })} ${now.getFullYear()}` },
        all: { title: 'Reporte general', subtitle: 'Todas las ventas registradas' }
    };
...
function renderSales() {
    ...
    tbody.innerHTML = [...filtered].reverse().map((sale, i) => {
        const date = new Date(sale.date);
        ...
        return `
            <tr class="border-t dark:border-[#2a2a2a] hover:bg-gray-50 dark:hover:bg-[#262626] ${i % 2 === 0 ? 'dark:bg-[#222]' : 'dark:bg-[#1f1f1f]'}">
                ...
                <td class="px-4 py-3 text-gray-600 dark:text-gray-300">${sale.student}</td>
                <td class="px-4 py-3 text-gray-500 dark:text-gray-400 text-xs"><ul class="list-disc list-inside">${products}</ul></td>
                <td class="px-4 py-3">
                    <span class="px-2 py-1 rounded text-xs font-semibold ${sale.paymentMethod === 'CASH' ? 'bg-gray-100 dark:bg-[#2a2a2a] text-gray-600 dark:text-gray-400' : 'bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300'}">
                        ${method}
                    </span>
                </td>
